...
def show_output(st , question):
        ...
        if question.lower() in ["hello", "hi", "who are you"]:
            message = {'role': "assistant", 'content': service}
        ...
        st.session_state.messages.append(message)
        show_assistant_message(st , role='assistant', avatar= assistant_icon , response_md  = service_data, response_df = response_df)
            ...
            with st.chat_message("assistant", avatar = avatar):
                if not response_df.empty:
                    ...
                    with tab1:
                        ...
                        #     return()
                        try: 
                            grid_response = build_aggrid(response_df)
                        except:
                            st.markdown("Data is already displayed. Please refer above")
                            return()
                    ...
                    with tab2:
                        if len(paginated_data.columns) == 2 and paginated_data.dtypes[1] in ['int64', 'float64'] or st.session_state.load_data:
                            ...
                            graphs = st.selectbox("Select Chart:", ["Bar Graph", "Line Plot", "Histogram"] ,key = st.session_state.key)
                    ...
                    st.markdown(response_md)
#Build container to display data
def build_aggrid(response_df = None ):
    # if response_df.columns.has_key("__metadata__"):
    response_df = response_df.drop("__metadata", axis=1)  # remove metadata column if exists
    print(response_df)
    ...
    grid_response = AgGrid(
        ...
        fit_columns_on_grid_load=True,
        update_mode='MODEL_CHANGED')                    
    del gb # clear object after displaying to avoid any exception 
